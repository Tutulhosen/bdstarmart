...
class TopHeaderController extends Controller
{
    ...
    public function update_page($id){
        $data['top_header_info']=DB::table('meta')->where('id', $id)->first();
    ...
    public function update(Request $request){
        ...
        $meta = DB::table('meta')->where('id', $request->id)->value('fav_icon');
        ...
        if (!empty($image)) {
            ...
            $update = DB::table('meta')->where('id', $request->id)->update([
                'fav_icon' =>$imageName,
                'title' => $name,
            ]);
        }else {
    ...
    public function delete($id){
        ...
        $delete=DB::table('meta')->where('id', $id)->delete();
        if ($delete) {
            return response([
                'status' =>true,
    ...
    public function status($id){
        $top_header_id=DB::table('meta')->where('id', $id)->first();
        if ($top_header_id->status==1) {
            
           $update= DB::table('meta')->where('id', $id)->update([
                'status'    =>0,
           ]);
            
        }elseif ($top_header_id->status==0) {
            $get_status=DB::table('meta')->where('status',1)->get();
